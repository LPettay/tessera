...
v0.2 in progress. v0.1 surface is in place (types, SVG renderer, page-citizenship, oscillate + spin animations). The `BackgroundGenerator<Config>` primitive landed in PR #7. The first concrete generator (`sunburst` — quantized radial gradient + rotating ray overlay) ships in this PR; `halftone` follows next.
