...
| `build.ts` | Static build — produces `dist/mug/index.html` + `dist/mug/main.js` for GitHub Pages (ADR 0017). Same `Bun.build` options as `dev.ts`. Run via `bun run build:mug`. |
